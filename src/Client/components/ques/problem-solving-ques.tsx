...
const ProblemItem: React.FC<ProblemItemProps> = ({
  ...
  <div className="rounded-lg w-full flex flex-col justify-center items-center gap-4">
    ...
    <div className="dark:bg-[#18181b] dark:text-white w-full"><MdEditor value={problem.question} onChange={onQuestionChange} /></div>
    ...
    <div className="w-full flex justify-between">
      ...
      <div>
        <Button variant="flat" startContent={<Icon icon="lucide:plus" />} onPress={onAddTestCase} className="w-full">
          Add Test Case
        </Button>
      </div>
    </div>
...
const TestCaseItem: React.FC<TestCaseItemProps> = ({ testCaseIndex, testCase, onDelete, onRefresh, onInputChange }) => (
  <div className="w-full flex gap-2">
    <div className="flex flex-col gap-2">
      <Button isIconOnly variant="flat" onPress={() => onDelete(testCaseIndex)}>
...
const FormFooter: React.FC<FormFooterProps> = ({ totalPages, currentPage, onPrevious, onNext }) => (
...
export default function ProblemSolvingForm({ examId }: ProblemSolvingFormProps) {
  ...
  const updateProblem = (index: number, updatedProblem: Problem) => {
    setProblems((prev) => [...prev.slice(0, index), updatedProblem, ...prev.slice(index + 1)])
  }
          ...
          <Form className="w-full flex flex-col gap-4 p-5 border-none" onSubmit={handleSaveProblem}>
            {currentProblems.map((problem, index) => (
              <ProblemItem
                ...
                onQuestionChange={(value) => handleQuestionChange(currentProblemIndex + index, value)}
                onPointsChange={(value) => handlePointsChange(currentProblemIndex + index, value)}
                onDifficultyChange={(value) => handleDifficultyChange(currentProblemIndex + index, value)}
