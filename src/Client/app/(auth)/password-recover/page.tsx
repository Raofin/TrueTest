...
const PasswordRecoverPage = () => {
    ...
    const onSubmit = async (data: { otp: string }) => {
        try {
            ...
            if (verifyResponse.data.isValidOtp) {
                ...
                router.push(
                    `/password-recover/reset-password?email=${formData.email}&otp=${data.otp}`
                );
            } else {
    ...
    return (
        <div className="flex items-center justify-center ">
            <Card
                className="px-8 py-5 shadow-none bg-white dark:bg-[#18181b] "
                style={{ maxWidth: "370px", flexShrink: 0 }}
            >
                <form
                    id="#"
