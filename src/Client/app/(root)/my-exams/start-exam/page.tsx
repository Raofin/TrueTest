...
import { convertUtcToLocalTime, FormattedDateWeekday, formatTimeHourMinutes } from "@/components/DateTimeFormat";
...
export default function StartExam() {
  ...
  const currentExam: ExamData | undefined = examDataString ? JSON.parse(examDataString) : undefined;
  ...
  const handleStartExam = async () => {
    try {
      ...
      if (navigator.clipboard && navigator.permissions) {
        const clipboardPermission = await navigator.permissions.query({ name: "clipboard-read" as PermissionName });
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">
                  {currentExam?.title}
                  <span className={`ml-2 text-sm font-normal text-red-500`}>{currentExam?.status}</span>
                </h1>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-red-500"></div>
                  <p className="min-w-[100px] text-gray-700 dark:text-gray-300">
                    {isClosed ? "Exam Closed" : `${hoursLeft}h ${minutesLeft}m left`}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <div>
                  ...
                  <FormattedDateWeekday date={currentExam?.closesAt} />
                </div>
                ...
                <div>
                  ...
                  {currentExam?.opensAt && convertUtcToLocalTime(currentExam.opensAt)}
                </div>
                <div className="text-right">
                ...
                <div>
                  <span className="text-gray-500 dark:text-gray-400">Closes at:</span>{" "}
                  {currentExam?.closesAt && convertUtcToLocalTime(currentExam.closesAt)}
                </div>
                <div className="text-right">
                  <span className="text-gray-500 dark:text-gray-400">MCQ:</span>{" "}
              ...
              <div className="space-y-4">
                ...
                <ol className="list-decimal list-inside space-y-3 text-gray-700 dark:text-gray-300">
                  ...
                  <li>
                    The exam will automatically switch you to full-screen mode. Please do not try to change it.
                  </li>
                  <li>
                    Do not attempt to copy or paste anything during the exam. It will be detected and flagged.
                  </li>
                </ol>
              </div>
