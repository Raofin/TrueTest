      ...
      <div>
        <div className="flex justify-center items-end text-center my-7">
          ...
          <div className="flex flex-col gap-2 ml-2">
            <Input name="firstName" placeholder="Enter first name" value={currentFormData.firstName} onChange={handleChange} />
            <Input name="lastName" placeholder="Enter last name" value={currentFormData.lastName} onChange={handleChange} />
          </div>
        </div>
        <div className="w-[550px] grid gap-4">
          <Textarea name="bio" placeholder="Write your bio here." value={currentFormData.bio} rows={5} onChange={handleChange} />
          <div className="grid grid-cols-2 gap-2">
            <Input name="instituteName" placeholder="Enter institute name" value={currentFormData.instituteName} onChange={handleChange} />
            <Input name="phoneNumber" placeholder="Enter phone number" value={currentFormData.phoneNumber} onChange={handleChange} />
          </div>
          <div className="space-y-2">
            {currentFormData.profileLinks.map((link, index) => (
              <div key={index} className="flex gap-2 items-center">
                <Input placeholder="Name" value={link.name} onChange={e => handleSocialLinkChange(index, 'name', e.target.value)} />
                <Input placeholder="Social Link" value={link.link} onChange={e => handleSocialLinkChange(index, 'link', e.target.value)} />
            ...
            <button type="button" onClick={handleAddSocialLink} className="text-blue-500 hover:text-blue-700 flex items-center gap-1">
